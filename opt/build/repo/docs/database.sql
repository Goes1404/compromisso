...
CREATE TABLE IF NOT EXISTS public.profiles (
  ...
  profile_type TEXT DEFAULT 'student', -- 'etec', 'uni', 'teacher', 'admin', etc.
  institution TEXT,
  course TEXT,
...
-- 7. FUNÇÃO DE CRIAÇÃO AUTOMÁTICA DE PERFIL (TRIGGER)
...
CREATE POLICY "Chat: ver próprias" ON public.direct_messages FOR SELECT USING (auth.uid() = sender_id OR auth.uid() = receiver_id);
CREATE POLICY "Chat: enviar" ON public.direct_messages FOR INSERT WITH CHECK (auth.uid() = sender_id);

ALTER TABLE public.activity_logs ENABLE ROW LEVEL SECURITY;
CREATE POLICY "Logs: leitura admin" ON public.activity_logs FOR SELECT USING (true);
CREATE POLICY "Logs: sistema insert" ON public.activity_logs FOR INSERT WITH CHECK (true);
